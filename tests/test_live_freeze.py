...
# ── Das Zuruecknehmen des Standbildes ─────────────────────────────────────────
#
# Festhalten allein reicht nicht. Zwischen dem Moment, in dem gphoto2 das
# USB-Geraet uebernimmt (Live-View aus, Bild steht), und dem Verschluss liegt
# die Verzoegerung der Kamera — auf der EOS 700D rund 1,1 s, beim ersten Schuss
# nach Ruhe eher 1,9 s. Ein gestochen scharfes, unbewegtes Bild des Gastes
# liest sich in dieser Zeit wie "Foto ist im Kasten": er entspannt sich, und
# genau dann faellt der Verschluss. Unscharf und abgedunkelt behauptet dasselbe
# Bild nichts mehr.


class VeilProbe:
    """Nur der Schleier, ohne Fenster und ohne Live-Reader."""

    _VEIL_BLUR_DIV = ui_mod.UI._VEIL_BLUR_DIV
    _VEIL_ALPHA = ui_mod.UI._VEIL_ALPHA
    _veiled_surface = ui_mod.UI._veiled_surface

    def __init__(self):
        self._live_hold_at = 0.0
        self._veil_key = None
        self._veil_surf = None


def _mean_brightness(surf):
    import pygame
    return float(np.mean(pygame.surfarray.array3d(surf)))


def test_the_hold_says_that_it_is_a_hold(hold):
    """Der Schleier haengt an diesem Schalter — er muss beides koennen."""
    hold.reader.frame = _bright(120)
    hold._live_frame_rgb()
    assert hold._live_is_hold is False

    hold.reader.is_moving = False
    hold.tick(1.0)
    hold._live_frame_rgb()

    assert hold._live_is_hold is True


def test_the_held_picture_is_darkened_and_blurred():
    """Sichtbar zurueckgenommen — sonst sieht das Standbild aus wie ein
    lebendes Bild, und der Gast haelt die Aufnahme fuer erledigt."""
    probe = VeilProbe()
    # Ein Motiv mit harter Kante: an ihr zeigt sich die Unschaerfe.
    frame = np.zeros((480, 640, 3), dtype=np.uint8)
    frame[:, 320:] = 200

    surf = probe._veiled_surface(frame, 640, 480)

    assert _mean_brightness(surf) < 200 * 0.5 * 0.75, "deutlich dunkler"
    # Mitten auf der Kante steht jetzt ein Uebergang statt eines Sprungs.
    edge = [surf.get_at((x, 240))[0] for x in range(300, 341)]
    assert len(set(edge)) > 3, "die Kante muss verlaufen, nicht springen"


def test_the_veil_is_computed_once_per_standstill():
    """Die Schleife laeuft mit 30 fps, das Standbild aendert sich nicht.

    Ohne Cache kostete jede Aufnahme rund sechzig Weichzeichner auf vollem
    Schirm — auf dem Pi genau in der Sekunde, in der die Box sonst nichts
    falsch machen darf.
    """
    probe = VeilProbe()
    frame = _bright(120)

    first = probe._veiled_surface(frame, 640, 480)
    again = probe._veiled_surface(frame, 640, 480)
    assert again is first

    # Frisches Bild: neuer Zeitstempel des Halts, also neuer Schleier.
    probe._live_hold_at = 1.0
    assert probe._veiled_surface(frame, 640, 480) is not first
